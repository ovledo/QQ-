...
async def health_check():
    """健康检查端点 - 前端心跳检测用"""
    return {"status": "ok", "time": datetime.now().isoformat(), "db_path": str(DB_PATH), "db_source": DB_SOURCE}
...
def update_category_item(cid: int, item: CatItemUpdate, db: Session = Depends(get_db)):
    ...
    changes = _diff_row(row, item.model_dump())
    if changes:
...
def dashboard(db: Session = Depends(get_db)):
    ...
    for t in all_tx:
        ...
        if t.type == "买币":
            ...
            if is_this_year: year_profit += trade_flow
    ...
    coin_balance = coin_from_initial + coin_from_tx + coin_from_items + coin_from_dungeons + coin_calibration - coin_misc_expense
    ...
    _prices = db.query(PriceHistory).filter(PriceHistory.deleted_at == None).order_by(PriceHistory.timestamp).all()
    _monthly_rate = {}
    for _p in _prices:
        if _p.timestamp:
            _monthly_rate[_p.timestamp.strftime("%Y-%m")] = _p.price_10e or _p.price or 0
    for dg in all_dungeons:
        if not dg.timestamp:
            continue
        dg_month_key = dg.timestamp.strftime("%Y-%m")
        dg_rate = _monthly_rate.get(dg_month_key, 0)
        if dg_rate == 0:
            for _prev_m in sorted(_monthly_rate.keys(), reverse=True):
                if _prev_m <= dg_month_key:
                    dg_rate = _monthly_rate[_prev_m]
                    break
        if dg_rate == 0:
            dg_rate = latest_price
    ...
    _write_asset_snapshot(db, datetime.now().strftime("%Y-%m"), {
        ...
        "coin_hold": round(coin_balance, 2),
    ...
    return {
        ...
        "coin_from_initial": round(coin_from_initial, 2),
        "coin_from_tx": round(coin_from_tx, 2),
        "coin_from_items": round(coin_from_items, 2),
        "coin_from_dungeons": round(coin_from_dungeons, 2),
        "coin_calibration": round(coin_calibration, 4),
        "coin_misc_expense": round(coin_misc_expense, 4),
        "items_coin_spent": round(items_coin_spent, 2),
        "items_coin_received": round(items_coin_received, 2),
        ...
        "dungeon_total_coin": round(dungeon_total_coin, 2),
        "dungeon_total_rmb": round(dungeon_total_coin * latest_price, 2),
...
def monthly_summary(db: Session = Depends(get_db)):
    ...
    def _ensure_month(key: str):
        if key not in months:
            months[key] = {"month": key, "recharge": 0, "cashout": 0,
                           "trade_in": 0, "trade_out": 0, "dungeon": 0, "consumed": 0}
    ...
    for d in all_dg:
        ...
        _ensure_month(key)
        # 优先使用当月币价，无则向前找最近的，最终回退到最新币价
        rate = monthly_rate.get(key, 0)
        if rate == 0:
    ...
    for key in sorted(months.keys()):
        ...
        monthly_pnl = trade_pnl + m["dungeon"] - m.get("consumed", 0)
        cum_pnl += monthly_pnl
        result.append({
            "month": m["month"],
...
def month_compare(m1: str, m2: str, db: Session = Depends(get_db)):
    ...
    def _month_rate(month_key: str) -> float:
    ...
    def _month_data(month_key: str) -> dict:
        recharge = cashout = trade_in = trade_out = dg_rmb = consumed = 0.0
        tx_count = dg_count = 0
        for t in all_tx:
        ...
        for d in all_dg:
            if not d.timestamp or d.timestamp.strftime("%Y-%m") != month_key:
                continue
            dg_count += 1
            dg_rmb += (d.revenue_coin or 0) * _month_rate(month_key)

        for item in all_items:
            if item.type != '买入' or getattr(item, 'holding_status', '') != '已消耗':
                continue
            ref_time = getattr(item, 'status_changed_at', None) or item.timestamp
        ...
        total_pnl = trade_pnl + dg_rmb - consumed
        return {
            "month": month_key,
            "tx_count": tx_count,
    ...
    diff = {}
    for k in ['tx_count', 'recharge', 'cashout', 'trade_in', 'trade_out', 'trade_pnl', 'dg_count', 'dg_rmb', 'consumed', 'total_pnl']:
...
def export_category_items(db: Session = Depends(get_db)):
    ...
    buf.write('\ufeff')
    w = csv.writer(buf)
    w.writerow(["ID", "时间", "分类", "类型", "物品名", "币价(亿)", "购买时币价(元/亿)", "数量", "预估价值(亿)", "持有状态", "备注"])
    for r in rows:
        w.writerow([r.id, r.timestamp.strftime("%Y-%m-%d %H:%M") if r.timestamp else "",
                    r.category, r.type, r.item_name, r.coin_price, r.purchase_rate,
...
def batch_update_items(body: BatchUpdateBody, db: Session = Depends(get_db)):
    if body.holding_status is None and body.category is None:
